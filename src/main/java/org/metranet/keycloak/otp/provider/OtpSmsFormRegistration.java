...
public class OtpSmsFormRegistration implements FormAction, FormActionFactory {
    ...
    private UserModel getUserByMobileNumber(ValidationContext context, String mobilePhone) {
        List<UserModel> listuser = context.getSession().users()
                .searchForUserByUserAttributeStream(context.getRealm(), OtpSmsConstant.ATTR_PHONE_NUMBER_ADMIN,
                        mobilePhone)
    ...
    private String validity(ValidationContext context, String mobileNumber, String code) {
        ...
        MultivaluedMap<String, String> formData = context.getHttpRequest().getDecodedFormParameters();
        String number = formData.getFirst(OtpSmsConstant.ATTR_PHONE_NUMBER);
        // formData.putSingle("username", number);
        logger.info(formData);
        UserModel user = getUserByMobileNumber(context, number);
        if (user != null) {
            return "";
        }
        ...
        // OtpSmsConstant.HTTP_AUTH_HOST + "/auth/realms/akuid/otpsms/ck/" +
        // mobileNumber + "/" + code);
